...
from sawtooth_bbci.processor.handler import make_bbci_address
...
def create_bbci_transaction(verb, name, value, deps, signer):
    """Creates a signed bbci transaction.
    ...
    Returns:
        transaction (transaction_pb2.Transaction): the signed bbci
            transaction
    """
    payload = BBCIPayload(
...
def add_create_batch_parser(subparsers, parent_parser):
    ...
    epilog = '''
    details:
     create sample batch(es) of bbci transactions.
     populates state with bbci key/value pairs
