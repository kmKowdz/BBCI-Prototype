...
from sawtooth_bbci.client_cli.workload.workload_generator import \
    WorkloadGenerator
from sawtooth_bbci.client_cli.workload.sawtooth_workload import Workload
from sawtooth_bbci.client_cli.create_batch import create_bbci_transaction
from sawtooth_bbci.client_cli.create_batch import create_batch
...
BBCIState = namedtuple('BBCIState', ['name', 'url', 'value'])
    ...
    def __init__(self, delegate, args):
        ...
        if args.key_file is not None:
            try:
                ...
                raise BBCICliException(str(pe)) from pe
            except IOError as ioe:
                raise BBCICliException(str(ioe)) from ioe
        else:
            self._signer = crypto_factory.new_signer(
            ...
            if code == 202:
                with self._lock:
                    self._pending_batches[batch_id] = \
                        BBCIState(name=name, url=url, value=0)
...
def do_workload(args):
    """
    Create WorkloadGenerator and BBCIWorkload. Set BBCI workload in
    generator and run.
    """
    try:
        args.auth_info = _get_auth_info(args.auth_user, args.auth_password)
        generator = WorkloadGenerator(args)
        workload = BBCIWorkload(generator, args)
