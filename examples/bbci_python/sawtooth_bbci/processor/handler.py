...
BBCI_ADDRESS_PREFIX = hashlib.sha512(
    FAMILY_NAME.encode('utf-8')).hexdigest()[0:6]


def make_bbci_address(name):
    return BBCI_ADDRESS_PREFIX + hashlib.sha512(
...
class BBCITransactionHandler(TransactionHandler):
    ...
    def apply(self, transaction, context):
        ...
        updated_state = _do_bbci(verb, name, value, state)
...
def _get_state_data(name, context):
    address = make_bbci_address(name)
...
def _do_bbci(verb, name, value, state):
    verbs = {
        'set': _do_set
    }
