...
def load_default_bci_config():
    """
    Returns the default BCIConfig
    """
    return BCIConfig(
...
def load_toml_bci_config(filename):
    """Returns a BCIConfig created by loading a TOML file from the
    filesystem.
    ...
    Returns:
        config (BCIConfig): The BCIConfig created from the stored
            toml file.
...
def merge_bci_config(configs):
    """
    Given a list of BCIConfig objects, merges them into a single
    BCIConfig, giving priority in the order of the configs
    (first has highest priority).

    Args:
        config (list of BCIConfigs): The list of bci configs that
            must be merged together

    Returns:
        config (BCIConfig): One BCIConfig that combines all of the
            passed in configs.
    """
    connect = None
